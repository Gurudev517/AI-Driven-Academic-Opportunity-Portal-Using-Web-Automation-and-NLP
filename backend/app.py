...
# This ensures the app finds the database and uploads folder regardless of where you run it from
...
# Full Institute List (As specified in Methodology Section 3.1) [cite: 13, 29]
INST_MAP = {
...
def get_data():
    ...
    cursor = conn.cursor()
    # Clean up expired posts (As specified in Methodology Section 3.1) [cite: 15, 49, 75]
...
# FIXED ENDPOINT FOR MATCHER [cite: 19, 65]
@app.route('/matcher')
def matcher():
...
def match_resume():
    ...
    if not df.empty:
        for item in df.to_dict(orient='records'):
            # Simple keyword matching as implemented in your current code logic [cite: 20, 55]
...
# --- CHATBOT ROUTE (AI Feature) [cite: 18, 58] ---
...
def chat():
    ...
    # 2. If no intent matched, assume it's a SEARCH QUERY (AI Discovery Module) [cite: 29]
...
if __name__ == '__main__':
    init_db()
    # Runs on port 5001 as specified [cite: 76]
